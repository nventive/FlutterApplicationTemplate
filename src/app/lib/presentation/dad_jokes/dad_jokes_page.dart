...
final class DadJokesPage extends ConsumerWidget {
  ...
  static final dadJokesProvider = StreamProvider<List<DadJoke>>((ref) async* {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final dadJokesAsyncValue = ref.watch(dadJokesProvider);
    return dadJokesAsyncValue.when(
      data: (dadJokes) {
        return ListView.builder(
          itemCount: dadJokes.length,
          itemBuilder: (context, index) {
            return DadJokeListItem(dadJoke: dadJokes[index]);
          },
        );
      },
